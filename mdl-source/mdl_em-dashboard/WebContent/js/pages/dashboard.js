...
angular.module('newApp').factory('dashboardService', function ($location,$filter) {

    var dashboard = {};
    
    dashboard.triggerNotification = function(issueCount) {
        /* Notifications, demo purpose */
    	if(typeof issueCount != "undefined"){
            notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#all_issues"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14">'+ issueCount +' issues reported!</h4><p><a href="#" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
            if(typeof dashboard.notifyflag != 'undefined'){
            	dashboard.notifyflag.close();
            }
            dashboard.notifyflag = generateNotifDashboard(notifContent);    		
    	}
    }
    ...
    dashboard.charts = function(id,coordinates,servicename,legendid) {
    	// panel-stat-chart, visitors-chart
        general_xarr = []; // data for x axis 
        g_datasets = []; // Data of all charts  
        checksum=[]; //to obtain unique records of issuehour
        checksum_graph=[]; // Grapgh details in sorted order
        graph_name_list=[]; // list of Graph names
        var Field_val = '';
    	var min="";
    	var max=0;
       ...
       if(coordinates.length>0){
	       ...
	       $.each(coordinates, function(key1, item1) {//Define max / min hour and separate data per election 	   	    		   
	    	   if(typeof min == 'undefined' || min.length==0)
	    		   min=eval(item1.issuehour);
	    	   else if(item1.issuehour<min)
	    		   min =eval(item1.issuehour);
	    	   
	    	   if(item1.issuehour>max)
	    		   max = eval(item1.issuehour);    

    		   if(typeof graph_name_list[item1.electionname] === 'undefined'){//If new election
    			   graph_name_list[item1.electionname]=1; // Track new election
    			   checksum_graph.push($filter('filter')(coordinates, { electionname: item1.electionname }));//Separate data per election
    		   }
	    	       	    		
	        });
	       
	       dashboard.legendArr = []; 
	       var xlblarr = []; // X axis data
	       
	       $.each(checksum_graph, function(key, electionArray) {// Go through election list
	           var ylblarr = []; // refresh y axis data per election
	           net_sum = eval(0);
	           
		       for (i = min; i <= max; i++) {
		    	   if(xlblarr.length != (eval(max-min+1)))// X axis common for all graphs, so generate it once
		    		   xlblarr.push(i);
		    	   
		           var g_graph = $.grep(electionArray, function(v) {// if the hour is available, filter data of the hour 
		        	   if(typeof v !== 'undefined'){
		        		   return v.issuehour == i;   		   
		        	   }
		        	});
		           
		           if(g_graph.length==1){// If any data found for given hour		        	   
		    	       switch(servicename) {
		    	       		case 'getBallotIssueGraph':
		    	       			var y_val = g_graph[0].ballotpaperissued;
		    	       		break;
		    	       		case 'getPostalCollectedGraph':
		    	       			var y_val = g_graph[0].postalcollected;
		    	       		break;    	       	
		    	       }	        	   
		        	   
		        	   ylblarr.push(y_val);
		        	   net_sum = net_sum + eval(y_val);
		           }
		           else //No data found for given hour
		        	   ylblarr.push(0);
		    	}	           
			   
			   
	           colour_red = getRandom(252,1);
	           colour_green= getRandom(252,1);
	           colour_blue= getRandom(252,1);
	           var chart_color_val = "rgba(" + colour_red + ", "+ colour_green +", " + colour_blue + ",0.5)";
	           var legend_color_val = rgb2hex(chart_color_val);
	           g_datasets.push( // Final Graph info
	        	{
	        		label: electionArray[0].electionname,
	        		data: ylblarr,
	                fillColor: chart_color_val,
	                strokeColor: "rgba(" + colour_red + ", "+ colour_green +", " + colour_blue + ",0.7)",
	                pointColor: "rgba(" + colour_red + ", "+ colour_green +", " + colour_blue + ",1)",
	                pointStrokeColor: "#fff",
	                pointHighlightFill: "#fff",
	                pointHighlightStroke: "rgba(" + colour_red + ", "+ colour_green +", "+ colour_blue +",1)",
	        	}	   
	           );
	           dashboard.legendArr.push({name: electionArray[0].electionname, total: net_sum, color: legend_color_val});
	       });  
       }
    ...
    dashboard.timeToSolveIssueGraph = function(id,dataset) {
    	
    	if(dataset.length > 0) {//Plot the graph if more than one issue data is found
	    	var checksum = [];
	    	...
	    	var min="";
	    	var max=0;
	       $.each(dataset, function(key1, item1) {// Prepare Y axis data
	    	   if(typeof min == 'undefined' || min.length==0)
	    		   min=eval(item1.issuehour);
	    	   else if(item1.issuehour<min)
	    		   min =eval(item1.issuehour);
	    	   ...
	    	   if(item1.issuehour>max)
	    		   max = eval(item1.issuehour);    	
	    	   	checksum.push({ time:item1.issuehour, avgresolvetime:item1.avgresolvetime, issuecount: item1.reported });
	 			 // ylblarr.push(item1.issuehour);
	 	   }); 
	       if(min=="")
	    	   min=0;	
	       ...
	       for (i = min; i <= max; i++) {
	    	   xlblarr.push(i);
	           var g_graph = $.grep(checksum, function(v) {// Separate Graphs with given names
	        	   if(typeof v !== 'undefined'){
	        		   return v.time == i;   		   
	        	   }
	        	});
	           if(g_graph.length==1){
	        	   ylblarr.push(g_graph[0].avgresolvetime);
	           }
	           else
	        	   ylblarr.push(0);
	    	}
    ...
    dashboard.triggerIssueCount = function(chatCount, issueID) {
    	
    	//sessionStorage.issue_id = issueID;
    	
        /* Chats, demo purpose */
    	if(typeof chatCount != "undefined"){
    	/*	notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert">'+
    		'<div class="media-left"><a href="#/all_issues/manage/:'+issueID+'">'+
    		'<i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p">'+
    		'<h4 class="alert-title f-14"> A new chat has added!</h4><p>'+
    		'class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';*/
    	
            //notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#all_issues"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14">'+ chatCount +' chats added!</h4><p><a href="#" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
    		//notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#all_issues"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14"> A new chat has added!</h4><p><a href="#" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
    		notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#/all_issues/manage/'+issueID+'"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14"> A new chat has added!</h4><p><a href="#/all_issues/manage/'+issueID+'" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
    		//notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#all_issues/manage/'+issueID+'"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14"> A new chat has added!</h4><p><a href="#/all_issues/manage/'+issueID+'" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
            if(typeof dashboard.notifyflag != 'undefined'){
            	dashboard.notifyflag.close();
            }
            dashboard.notifyflag = generateNotifDashboardChat(notifContent, issueID);    		
    	}
    }
    
    dashboard.triggerAssignCount = function(assignCount, issueID, pollingstationID) {
        /* Chats, demo purpose */
    	if(typeof assignCount != "undefined"){
    		notifContent = '<div class="alert alert-dark media fade in bd-0" id="message-alert"><div class="media-left"><a href="#/all_issues/manage/'+issueID+'"><i class="fa fa-bell faa-ring animated"></i></a></div><div class="media-body width-100p"><h4 class="alert-title f-14"> A new issue is assigned to you!</h4><p><a href="#/all_issues/manage/'+issueID+'" class="f-12" style="margin-right:10px">Check them here.</a></p></div></div>';
    		if(typeof dashboard.notifyflag != 'undefined'){
            	dashboard.notifyflag.close();
            }
            dashboard.notifyflag = generateNotifDashboardChat(notifContent, issueID);    	   		
    ...
    function generateNotifDashboardChat(content, issueID) {
        var position = 'topRight';
        if ($('body').hasClass('rtl')) position = 'topLeft';
        ...
        var issuenotification = noty({
            ...
            callback: {
                ...
                onCloseClick: function () {
                    setTimeout(function () {
                    	$location.path('/all_issues/manage/'+issueID+'');
                    }, 500);
                }
            }
        });
