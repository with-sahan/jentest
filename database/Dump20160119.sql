...
INSERT INTO `election` VALUES (1,2,'Police Comminisoner Election','2016-01-01 00:00:00','2016-01-01 00:00:00',0,NULL),(2,2,'Mayor Election','2016-01-01 00:00:00','2016-01-01 00:00:00',0,NULL);
...
INSERT INTO `issue` VALUES (1,2,1,1,6,'Test',2,NULL,'',0,'2016-01-15 21:17:44',NULL),(2,2,1,1,6,'Very bad issue',1,NULL,'',0,'2016-01-18 13:21:01',NULL),(3,2,1,NULL,6,'So much violance',1,NULL,'',0,'2016-01-18 13:31:47',NULL);
...
INSERT INTO `open_close_election` VALUES (4,2,1,5,1,1),(7,2,1,4,1,1),(8,2,1,4,0,2),(9,2,1,5,1,2);
...
INSERT INTO `polling_station` VALUES (1,2,'Polling Station 1','','',4,'2016-01-13 20:54:51',NULL),(2,2,'Derry Booth Polling Station','22,Derry Cold','KJUUU',4,'2016-01-18 12:46:25',NULL);
...
INSERT INTO `polling_station_election` VALUES (1,2,1,1,1,1,'02:33:36',NULL,'2016-01-16 13:39:46'),(2,2,2,2,1,1,'10:55:55',NULL,'2016-01-16 13:39:46');
...
INSERT INTO `tracking` VALUES (1,2,1,2,'4.33222','2.34333',2,'address','2016-01-18 18:53:32','2016-01-18 19:47:23',NULL);
...
INSERT INTO `user_election` VALUES (1,2,2,1,1),(2,2,2,2,2);
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `closetrack`(in token varchar(255),in electionid INT,in stationid int)
BEGIN
	declare spcode varchar(45);
    declare username varchar(45);
    declare orgcode varchar(45);
    ...
    if (security.fn_validate_token(token)=1) then
        ...
        where organization_id=orgid and pollingstationid=stationid and election_id=electionid;
        select "success" as response;
    else
		select "unauthorized" as response;
    end if;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `starttrack`(in token varchar(255),in electionid INT,in stationid int)
BEGIN
	declare spcode varchar(45);
    declare username varchar(45);
    declare orgcode varchar(45);
    ...
    select id into orgid from subscription.organization where code = orgcode;
    if (security.fn_validate_token(token)=1 and security.fn_validate_permission(token,spcode,'PSM')) then
		if(select exists(select t.*  from psm.tracking t
        inner join psm.election e on t.election_id=e.id
        inner join psm.polling_station p on t.pollingstationid=p.id
        inner join subscription.organization o on t.organization_id=o.id
        where o.code=orgcode and t.election_id=electionid and t.pollingstationid=stationid)) then
			...
			values
			(orgid,stationid,electionid,'address',CURRENT_TIMESTAMP,1);
			select "success" as response;
        end if;
    else
		select "unauthorized" as response;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `updatetrack`(in token varchar(255),in electionid INT,in stationid int,in longtitude varchar(45) ,in latitude varchar(45) )
BEGIN
	declare spcode varchar(45);
    declare username varchar(45);
    declare orgcode varchar(45);
...
-- Dump completed on 2016-01-19  9:35:31
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
INSERT INTO `access_token` VALUES (10,2,2,'admin|DERRY|3e90b5645f72ef180ceb2f597fa2d7bd','2016-01-13 05:08:47','2016-01-14 05:08:47'),(11,2,2,'admin|DERRY|1a4d75813c6f8cde22169cb99e8de5b7','2016-01-14 10:39:35','2016-01-15 10:39:35'),(12,2,2,'admin|DERRY|bd1a59798162d455256eadce55b380d6','2016-01-15 11:12:51','2016-01-16 11:12:51'),(13,2,2,'admin|DERRY|dcfaf92ba60e4f1c7ea161c7df961caf','2016-01-16 17:41:23','2016-01-17 17:41:23'),(14,2,2,'admin|DERRY|fa128b144b8bd02bf9a01a0ab49e129e','2016-01-18 10:03:22','2016-01-19 10:03:22');
...
INSERT INTO `role_permission` VALUES (1,2,1,1),(2,2,1,2),(3,2,1,3),(4,2,1,4),(5,2,1,5),(6,2,1,6),(7,2,1,7),(8,2,1,8),(9,2,1,9),(10,2,1,10),(11,2,1,11),(12,2,1,12),(13,2,1,13),(14,2,1,14),(15,2,1,15),(16,2,1,16),(17,2,1,17);
...
-- Dump completed on 2016-01-19  9:35:47
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
-- Dump completed on 2016-01-19  9:35:58
