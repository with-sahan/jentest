...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `updatepassword`(IN token VARCHAR(45),
...
-- Dump completed on 2016-04-11 12:38:36
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
in person_name varchar(255), in updatetime int)
BEGIN
	declare spcode varchar(45);
    declare username varchar(45);
    declare orgcode varchar(45);
    ...
    select id into orgid from subscription.organization where code=orgcode;
    if (security.fn_validate_token(token)=1 ) then
		update psm.election_stats set uplifting_person_name=person_name where organization_id=orgid
        and electionid=electionid and polling_station_id=stationdid
        and (HOUR(updatedon) = updatetime) 
        and (DATE(current_timestamp()) = DATE(updatedon));
        select 'success' as response;
    else
		select 'unauthorized' as response;
    end if;
...
BEGIN
    ...
    if (security.fn_validate_token(token)=1) then
		...
		select hv.id as hierarchy_value_id, 
        hv.value as hierarchy_value_name,
        'success' as response 
        from psm.hierarchy_value hv
        inner join psm.hierarchy h on h.name = 'Place'
        where h.organization_id = organizationid
        and hv.organization_id = organizationid
        order by hv.value;
...
BEGIN
    ...
    if (BPAIdentifier=0) then
        ...
        SUM(es.ten_spoilt_ballots) as ten_spoilt_ballots2,
        'success' as response
		from psm.election_stats es 
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `getpsichecklist`(in token varchar(255), in place_id int )
BEGIN
    ...
    if (security.fn_validate_token(token)=1) then
		...
		select l.id as id, l.list_value as activity,lc.name as category, hv.id as hierarchy_value_id, hv.value as hierarchy_value_name, 'success' as response 
		from psm.list l inner join psm.list_category lc on l.list_category_id = lc.id 
		inner join subscription.organization org on lc.organization_id = org.id 
		inner join psm.hierarchy_value hv on hv.id = place_id 
		where lc.name like 'psi_checklist%' order by lc.list_order,l.list_order;
        
    else
		select null as id, null as activity,null as category, null as hierarchy_value_id, null as hierarchy_value_name, 'unauthorized' as response;
    end if;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
	...
	if (security.fn_validate_token(token)=1) then
		...
		elseif (BPAIdentifier=1) then
			...
			if(select exists 
				...
				and es.organization_id=orgid and es.electionid=electionid and es.polling_station_id=pollingstationid
                and UNIX_TIMESTAMP(DATE(updatedon)) = UNIX_TIMESTAMP(CURDATE());
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `updatepsichecklist`(token varchar(255), place_id int, activity_id int)
BEGIN
    ...
    if (security.fn_validate_token(token)=1) then
		...
		if(select exists 
			(
             ...
             and pcl.organization_id = orgid
			) 
            ) then
                /*update psi_check_list if record exists */
                update psm.psi_check_list pcl 
                inner join sm.hierarchy_value hv on pcl.place_id = hv.id
                set pcl.list_id = activity_id
                where pcl.place_id = place_id and
                pcl.organization_id = orgid;
			...
			/*insert record if doesnt*/
			insert into psm.psi_check_list(organization_id, list_id, place_id)
			values(orgid, activity_id, place_id);
...
-- Dump completed on 2016-04-11 12:39:40
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
-- Dump completed on 2016-04-11 12:39:52
