...
-- Dump completed on 2016-04-14  0:39:35
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
BEGIN
    ...
    if (security.fn_validate_token(token)=1 ) then
        ...
        tend_end_count - tend_start_count - ifnull(sum(ten_ballot_papers),0),
        
        ifnull(sum(ballotpaper2),0),
        ifnull(sum(spoilt_ballot2),0),
        end_count-start_count -ifnull(sum(ballotpaper2),0),
        ifnull(sum(ten_ballot_papers2),0),
        ifnull(sum(ten_spoilt_ballots2),0),
        tend_end_count - tend_start_count - ifnull(sum(ten_ballot_papers2),0)
        into
    ...
    if (security.fn_validate_token(token)=1) then
		...
		if (select exists 
			(
				...
				where pcl.place_id=placeid and l.organization_id = organizationid
...
-- Dump completed on 2016-04-14  0:40:22
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
-- Dump completed on 2016-04-14  0:40:43
