...
-- Dump completed on 2016-04-24 11:32:15
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `updateissue`(in token varchar(255),in issueid int,in userid int,in status int,in priority int,in comment varchar(1000),in watched int)
BEGIN
	declare spcode varchar(45);
    declare username varchar(45);
    declare orgcode varchar(45);
    ...
    if (security.fn_validate_token(token)=1 and security.fn_validate_permission(token,spcode,'PSM')) then
        ...
        if status=1 then
			update psm.issue set resolution_desc=comment,watched=watched,status=status,priority=priority,resolvedon=current_timestamp where id=issueid and organization_id=orgid;
        else
			update psm.issue set resolution_desc=comment,watched=watched,status=status,priority=priority where id=issueid and organization_id=orgid;
        end if;
...
-- Dump completed on 2016-04-24 11:34:32
-- MySQL dump 10.13  Distrib 5.7.9, for Win64 (x86_64)
--
...
-- Dump completed on 2016-04-24 11:34:53
