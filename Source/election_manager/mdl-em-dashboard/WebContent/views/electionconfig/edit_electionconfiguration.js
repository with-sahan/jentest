		...
		['$scope','toaster', 'mdl.mobileweb.service.json','mdl.dashboard.service.electionconfig','$rootScope', '$location', '$routeParams',
		 function ($scope, toaster, jsonService, elecconfigService, $rootScope, $location, $routeParams) {
			...
			vm.getElectionByID = function (election_Id) {
				elecconfigService.getElectionByID(election_Id,function (response) {
		            vm.oneOfElections = response;
		            //vm.oneOfElections.election_date = vm.oneOfElections.election_date.replace('+00:00', '');
		            vm.oneOfElections.election_date = vm.oneOfElections.election_date.substring(0,10);
		            //vm.oneOfElections.election_start_time = vm.oneOfElections.election_start_time.replace(':00.000+00:00', '');
		            vm.oneOfElections.election_start_time = vm.oneOfElections.election_start_time.substring(0,5);
		            //vm.oneOfElections.election_end_time = vm.oneOfElections.election_end_time.replace(':00.000+00:00', '');
		            vm.oneOfElections.election_end_time = vm.oneOfElections.election_end_time.substring(0,5);
		            ...
		            vm.selectCheckBox = vm.oneOfElections.bpa_identifier.toString();
		            
		            if (vm.oneOfElections.election_date == vm.oneOfElections.today_date.substring(0,10)) {
		            	if (((parseInt(vm.oneOfElections.election_start_time.substring(0,3), 10))) <= ((parseInt(vm.oneOfElections.today_time.substring(0,3), 10)))) {
			            	if (((parseInt(vm.oneOfElections.election_start_time.substring(3,5), 10))) <= ((parseInt(vm.oneOfElections.today_time.substring(3,5), 10)))) {
			            		//alert(parseInt(vm.oneOfElections.election_start_time.substring(3,5), 10) + " " + parseInt(vm.oneOfElections.today_time.substring(3,5), 10));
			            		//alert(parseInt(vm.oneOfElections.election_start_time.substring(0,3), 10) + " " + parseInt(vm.oneOfElections.today_time.substring(0,3), 10));
		            ...
		            } else if (vm.oneOfElections.election_date < vm.oneOfElections.today_date.substring(0,10)) {
		            	vm.disableTextBox = true;
		            } else {
		            	vm.disableTextBox = false;
		            }
		            ...
		            /*if (((vm.oneOfElections.election_date == vm.oneOfElections.today_date.substring(0,10))&&(vm.oneOfElections.election_start_time<=((vm.oneOfElections.today_time).slice(0,5)))) || vm.oneOfElections.election_date < vm.oneOfElections.today_date) {
		            	vm.disableTextBox = true;
		            }*/
					...
					vm.geocoder.geocode({'address': electionconfig.counting_center_address}, function(results, status) {
						if (status === google.maps.GeocoderStatus.OK) {
							...
							elecconfigService.updateElection(electionconfig, vm.latitude, vm.longitude,vm.selectCheckBox, function(response){
								...
								else if(response.response=="dataduplicate"){
									toaster.pop("info","This Election is already created!","");
									$rootScope.cui_blocker();
								}
								else if(response.response=="notcurrentdate"){
									toaster.pop("info","You can only update an Election from today onwards!","");
									$rootScope.cui_blocker();
								}
								else if(response.response=="notcurrenttime"){
