...
angular.module('newApp').controller('mdl.mobileweb.controller.recordprogressNew',
		['$rootScope', '$scope', 'mdl.mobileweb.service.dashboard', 'mdl.mobileweb.service.recordprogress', 'mdl.mobileweb.service.login', 'toaster', 'mdl.mobileweb.service.json', '$http', 'modalService', '$location','$filter',
		 function ($rootScope, $scope, dashboardService, recordprogressService, loginService, toaster, jsonService, $http, modalService, $location, $filter) {
		         ...
		         dashboardService.getpollingstationclosedstatus(stationId, function (response) {
		             ...
		             if (res.response == "success") {
		                 if ((res.open_status == 1) && (res.closed_status == 0)) {
		                     ...
		                     vm.stationstatus = false;
		                     toaster.pop("info", "Station Is Not Opened", "Record progress in not available in unopened stations.");
		                 }
		     ...
		     vm.showCloseButton = function (electionid, stationId) {
		         recordprogressService.showCloseButton(electionid, stationId, function (response) {
		        	 if (response.response == "success"){
		        		 if(vm.buttonflag ) {
		        			 vm.buttonshowDetails = response.buttonshow;
		        		 }
		        	 } else {
		        		 toaster.pop("error", "Error!", "Try again later");
		        	 }
		     ...
		     vm.currentStation = "notselected";//Holds the ID of current(selected) station
		     vm.getEList = function (stationid) {
		    	 vm.currentStation = stationid;
		     ...
		     vm.getStationList = function () {
		         dashboardService.getPollingStations(function (response) {
		             ...
		             vm.currentStation=vm.pollingStationList[0].id;
					 vm.selectedStation = vm.currentStation;
    				 vm.allStations = $filter('filter')(vm.pollingStationList, { closed_status: "1" });
		         })
		     };
		     vm.getStationList();
		     vm.allStations=[];
		     ...
		     vm.getProgressApiCall = function (electionid, stationId) {
		         recordprogressService.getProgress(electionid, stationId, function (response) {
		             ...
		             if (isAllSucess == "1") {
		                 ...
		                 $scope.arraylength = vm.progressSummery.length-1;
		     ...
		     vm.getPollingStationElectionDetails = function (electionid, pollingstationid, callback) {
		         recordprogressService.getPollingStationElectionDetails(function (response) {
		             vm.polling_Station_Election = response;
		             ...
		             if (vm.BPA_identifier == "1") {
			             if ((totals.ballotIssuedTotal) < 0) {
			                 ...
			                 if (!isValidInput)
			                     ...
			                     toaster.pop("error", "The number of ballot papers issued should be greater than the number of spoiled ballots", "");
			             }
	            	     ...
	            	     if ((totals.ballotIssuedTotal) < 0) {
	            	         ...
	            	         toaster.pop("error", "Your ballot paper count " + totals.ballotIssuedTotal + " is greater than total ballot turnout " + (vm.polling_Station_Election.ballotend - vm.polling_Station_Election.ballotstart + 1) + "!", "Please correct.");
	            	     }
	            	     else if ((totals.tendredballotIssuedTotal) < 0) {
	            	         toaster.pop("error", "Tendered Paper Issued should be greater than " + vm.polling_Station_Election.tenderstart + " and smaller than " + vm.polling_Station_Election.tenderend + "!", "");
	            	     }
	            	     else if ((totals.tendredballotIssuedTotal) > (vm.polling_Station_Election.tenderend - vm.polling_Station_Election.tenderstart + 1)) {
	            	         toaster.pop("error", "Your tendered paper count " + totals.tendredballotIssuedTotal + " is greater than total tendered turnout " + (vm.polling_Station_Election.tenderend - vm.polling_Station_Election.tenderstart + 1) + "!", "Please correct.");
	            	     }
	            	     else if ((totals.ballotIssuedTotal2) < 0) {
	            	         toaster.pop("error", "Ballot " + vm.bpNames.paper2 + " issued should be greater than " + vm.polling_Station_Election.ballotstart + " and smaller than " + vm.polling_Station_Election.ballotend + "!", "");
	            	     }
		     ...
		     vm.rc_completeprog_template = "views/templatemodal/record_progress_completeprogress.html";
		     vm.rc_completeprog = {
			         id: "rec_progress_completeprogresss",
			         header: "Confirmation",
			         body: "Please wait...loading",
			         closeCaption: "No",
			         saveCaption: "Yes",
			         save: function () {
			        	 modalService.close('rec_progress_completeprogresss');
			         },
			         close: function () { modalService.close('rec_progress_completeprogresss'); }
			}

		     vm.completeSequence = function(stationId){
		    	//Complete all the elections for given station ID, close station, generate BP and start tracking
		    	 recordprogressService.completeProgressSequence(stationId, function(response){
		    		 modalService.close('rec_progress_completeprogresss');
	                //  if (response.response == "finished") {
									if (response.response == "success") {

	        		         dashboardService.getPollingStations(function (response2) {
	        		             var resp = response2;
	        		             if (jsonService.whatIsIt(resp) == "Object") {
	        		                 vm.pollingStationList[0] = resp;
	        		             }
	            				 ...
	            				 vm.currentStation=vm.allStations[0].id;

        		                 vm.getEList(vm.currentStation);
			    				 vm.getProgress(vm.currentElection,vm.currentStation);
	        		         })
		     ...
		     vm.completeSequence2 = function(stationId){
			    	//Complete all the elections for given station ID, close station, generate BP and start tracking
			    	 recordprogressService.completeProgressSequence(stationId, function(response){

			    		 modalService.close('rec_progress_completeprogresss');
										if (response.response == "success") {
		                	 vm.completeProgressSequence = response;
		                     vm.checkElectionClosed = 1;
		                     toaster.pop("success", "RC completed, BPA generated, Station closed and GPS Tracking Started.", "");
		                     if(!$rootScope.trackingTimerStarted)
		                    	 $location.path('/gpstracking/enableTracking');
		                     vm.onRouteChangeOff();
		                 } else {
		                     toaster.pop("error", "Error!", "Try again later");
		                 }
		     ...
		     vm.callConfirmation = function (electionid, stationId,progressSummery) {
		    	 ...
		    	 vm.save(stationId, progressSummery, false, function () {
		    		 // Reset popup content
		    		 vm.rc_completeprog = {
		    				 id: "rec_progress_completeprogresss",
		    				 header: "Confirmation",
		    		 ...
		    		 if (vm.postalPacksTotalOnload() !== vm.getTotals().postalPacksTotal)
		    			 ...
		    			 $rootScope.cui_blocker(true);//UI blocker on
		    			 recordprogressService.getUnclosedElectionsbystationid(electionid, stationId, function(response){
		    				 vm.unclosedElections = [];
		    				 var isResponseEmpty = '';

		    				 if (jsonService.whatIsIt(response) == "Object") {
		    					 vm.unclosedElections[0]=response;
		    					 if(vm.unclosedElections[0].response=="empty")
		    						 isResponseEmpty = true;
		    					 else
		    						 isResponseEmpty = false;
		    				 }
		    				 else if (jsonService.whatIsIt(response) == "Array") {
		    					 isResponseEmpty = false;
		    					 vm.unclosedElections = response;
		    				 }

		    				 // Prepare the confirmation popup content as per the condition

		    				 if (!isResponseEmpty && vm.unclosedElections[0].response=="unauthorized")
		    					 toaster.pop("error", "", "Sorry, you are not permitted for this operation");
		    				 else if(isResponseEmpty){//No other elections for this station or rest of the elections are completed with RC
		    					 vm.rc_completeprog.body = "Note: All elections are completed. Please confirm your next operation.";
		    					 vm.rc_completeprog.closeCaption = "Close Station";
		    					 vm.rc_completeprog.saveCaption = "Cancel";
		    					 vm.rc_completeprog.save = function(){//Complete RC
		    						 //$scope.closeStation();
		    						 modalService.close('rec_progress_completeprogresss');
		    					 };
		    					 vm.rc_completeprog.close = function(stationId){// Complete sequence
		    						 if(vm.allStations.length==1) vm.completeSequence2(stationId);
		    						 else vm.completeSequence(stationId);
		    						 modalService.close('rec_progress_completeprogresss');
		    					 };
		    				 }
		    				 else {//More elections available
		    					 var allUncompleteElections = $filter('filter')(vm.unclosedElections, { status: "nolasthourrecord" });

		    					 if(allUncompleteElections.length>0){//incomplete elections (with "nolasthourrecord") remains
		    						 vm.rc_completeprog.body = "Are you sure you want to complete progress: No further Information can be recorded.";
		    						 vm.rc_completeprog.closeCaption = "Go to next election";
		    						 vm.rc_completeprog.saveCaption = "Cancel";
		    						 vm.rc_completeprog.save = function(){//Complete RC
		    							 //$scope.closeStation();
		    							 modalService.close('rec_progress_completeprogresss');
		    						 };
		    						 vm.rc_completeprog.close = function(stationId){//Go To other tab
		    							 vm.currentElection = allUncompleteElections[0].electionid;
		    							 vm.getProgress(vm.currentElection,stationId);
		    							 modalService.close('rec_progress_completeprogresss');
		    						 };

		    					 }
		    					 else {// All elections are having last hr records - complete
		    						 vm.rc_completeprog.body = "Note: All elections are completed. Please confirm your next operation.";
		    						 vm.rc_completeprog.closeCaption = "Close Station";
		    						 vm.rc_completeprog.saveCaption = "Cancel";
		    						 vm.rc_completeprog.save = function(){//Complete RC
		    				 ...
		    				 $rootScope.cui_blocker(false);//UI blocker close
		    				 modalService.load('rec_progress_completeprogresss');
		    			 });
		     ...
		     $scope.closeStation = function () {
		         if ($scope.rec_electionid.length > 0 && $scope.rec_stationId.length > 0) {
		             recordprogressService.closeStation($scope.rec_electionid, $scope.rec_stationId, function (response) {
		                 if (response.response == "success") {
		                     vm.closeStationDetails = response;
		     ...
		     vm.postalPackCollected = function (electionid, stationId) {
		         if ((isNaN(vm.getTotals().postalPacksTotal)) || (vm.getTotals().postalPacksTotal < 0)) {
		             ...
		             if (vm.postalPacksTotalOnload() === vm.getTotals().postalPacksTotal) {
		                 toaster.pop("success", "All Postal Packs have already been collected", "");
		     ...
		     vm.collectingPostalPacks = function (electionid, stationId, uplifting_person_name) {
		         recordprogressService.postalPackCollected_V2(electionid, stationId, uplifting_person_name, function (response) {
		             ...
		             if (response.response == "success") {
		                 vm.postalPackDetails = response;
		                 console.log('recordprogressService.postalPackCollected_V2')
		                 console.log(vm.postalPackDetails)
		     ...
		     $scope.completeprogressenable = function (){
		    	        $scope.$apply(function () {
		    	        	vm.buttonshowDetails=1;
		    	        	vm.buttonflag=false;
		    	        });
		     }
