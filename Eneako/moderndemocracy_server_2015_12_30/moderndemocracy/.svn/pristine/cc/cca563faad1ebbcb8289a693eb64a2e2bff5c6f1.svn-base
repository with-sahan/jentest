...
public class StationCollectedHandler extends ModernDemocracyHandler {

	private static final Logger logger = Logger.getLogger(StationCollectedHandler.class);
	...
	protected void handlePost(String target, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			logger.debug("POST StationCollected - stationId="+stationId);
			
			dao.insertDispatchedTime( source, stationId );
